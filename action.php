<?php
	...
	if ($action=="SelectLeave") {
		...
		$sql = "Select CPJ02 From ".$db."CPJ_FILE Where CPJ01 = '".$CQG03."' ";
	...
	if ($action=="CreateLeave") {
		...
		//判斷同一天是否重復請假
		$sql = "Select count(*) as counter From ".$db."cqg_file Where CQG01 = '".$_SESSION["MyMember"]["Code"]."' and to_char(CQG04, 'YYYY/mm/dd') = '".$CQG04."' and to_char(CQG05, 'YYYY/mm/dd') = '".$CQG05."' ";
		$rs = ConnectOracle($Oracle, $sql);
		oci_execute($rs);
		while ($row = oci_fetch_array($rs, OCI_ASSOC+OCI_RETURN_NULLS)) {
			foreach($row as $_key=>$_value) {
		...
		$CQG02 = 1;
		$sql = "Select CQG02 From ".$db."cqg_file Where CQG01 = '".$_SESSION["MyMember"]["Code"]."' and rownum <= 1 order by CQG02 desc ";
		$rs = ConnectOracle($Oracle, $sql);
		oci_execute($rs);
		while ($row = oci_fetch_array($rs, OCI_ASSOC+OCI_RETURN_NULLS)) {
			foreach($row as $_key=>$_value) {
		...
		for ($i=0; $i<$diff; $i++) {
			...
			//查班別&該班別上下班時間
			$sql = "Select CPY04, CPQ03, CPQ04, CPQ05, CPQ06, CPQ19 From ".$db."cpf_file, ".$db."cpq_file, ".$db."cpy_file Where CPF01 = CPY01 and CPY04 = CPQ02 and CPF01 = '".$_SESSION["MyMember"]["Code"]."' and CPY02 = '".$year."' and CPY021 = '".$month."' and CPY03 = '".$day."' and CPY05 = '0' ";
			$rs = ConnectOracle($Oracle, $sql);
			oci_execute($rs);
			while ($row = oci_fetch_array($rs, OCI_ASSOC+OCI_RETURN_NULLS)) {
				foreach($row as $_key=>$_value) {
				...
				$sql = "Insert into ".$db."CQH_FILE (CQH01, CQH02, CQH04, CQH05, CQH051, CQH06, CQH061, CQH07, CQH071, CQH08, CQH09, CQH10, CQH11) values ('$CQH01', '$CQH02', TO_DATE('".$CQH04."', 'YYYY/MM/DD'), TO_DATE('".$CQH05."', 'YYYY/MM/DD'), '$CQH051', TO_DATE('".$CQH06."', 'YYYY/MM/DD'), '$CQH061', '$CQH07', '$CQH071', '$CQH08', '$CQH09', '$CQH10', '$CQH11') ";
				//echo $sql."\n";
				$rs = ConnectOracle($Oracle, $sql);
				oci_execute($rs);			
	...
	//依日期找出上下班時間(未用到, 先保留)
	if ($action=="getWorkTime") {
		$date = str_filter($_POST["date"]);
		$empId = str_filter($_POST["empId"]);
		$date = explode("/", $date);		
		$year = $date[0];
		$month = $date[1];
		$day = $date[2];
